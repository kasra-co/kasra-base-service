...
var _ = require('lodash'),
	os = require('os'),
	fs = require( 'fs' ),
	logger = require( '@kasra/kasra-logger' ),
	db = require( '@kasra/kasra-db' ),
	https = require( 'https' ),
...
Initializer.prototype.initDb = function initDb(cli,loggerInst,callback) {
	...
	if(this.options.db) {
		db.init(_.extend(_.pick(this.options,'db'),{logger: loggerInst}),this.appConfig,function(err,dbConnections) {
			if(err) {
				return callback(err);
			}
			_.forOwn(dbConnections, function(dbInst) {
				loggerInst.info('Connected to ',dbInst.connstr);
			});

			dbConnections = _.mapValues(dbConnections,function(value,key) {
				return value.connection;
			});

			callback(null,cli,loggerInst,dbConnections);
		});
	} else {
...
Initializer.prototype.initService = function initService(cli,loggerInst,dbconn,callback) {
	...
	var ssl = this.options.ssl ? true : false;
	...
	var listener = function listener() {
		...
		loggerInst.info( 'Serving', 'at', app.get( 'host' ), 'on port', port, 'ssl:',ssl, 'testdb:', cli.test );
	...
	try {
		...
		if(this.options.ssl) {
			certificate = {
				key: fs.readFileSync(this.options.ssl.key ? this.options.ssl.key : '../../cert/'+hostname+'.key'),
				cert: fs.readFileSync(this.options.ssl.cert ? this.options.ssl.cert : '../../cert/'+hostname+'.crt')
			};
